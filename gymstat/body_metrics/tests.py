...
class RecordAPITests(APITestCase):
    def setUp(self):
        ...
        self.user_record = Record.objects.create(
            owner=self.user, metric=self.user_metric, value=75.0, datetime="2025-03-20T09:00:00Z"
        )

        self.admin_metric_record = Record.objects.create(
            owner=self.user, metric=self.admin_metric, value=180, datetime="2025-03-21T10:00:00Z"
        )

        self.other_user_record = Record.objects.create(
            owner=self.other_user, metric=self.other_user_metric, value=22.5, datetime="2025-03-22T11:00:00Z"
        )

        self.client.login(email="test@example.com", password="testpass")

    def test_get_records_for_user_metric(self):
        url = reverse("metrics:get-create-records") + f"?metric={self.user_metric.pk}"
        response = self.client.get(url)
    ...
    def test_get_records_for_admin_metric(self):
        url = reverse("metrics:get-create-records") + f"?metric={self.admin_metric.pk}"
        response = self.client.get(url)
    ...
    def test_get_records_metric_not_accessible(self):
        url = reverse("metrics:get-create-records") + f"?metric={self.other_user_metric.pk}"
        response = self.client.get(url)
    ...
    def test_create_record_admin_metric(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Record.objects.filter(owner=self.user, metric=self.admin_metric).count(), 2)
    ...
    def test_get_single_owned_record(self):
        url = reverse("metrics:get-edit-record", kwargs={"pk": self.user_record.pk})
        response = self.client.get(url)
    ...
    def test_get_single_not_owned_record(self):
        url = reverse("metrics:get-edit-record", kwargs={"pk": self.other_user_record.pk})
        response = self.client.get(url)
    ...
    def test_delete_not_owned_record(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertTrue(Record.objects.filter(pk=self.other_user_record.pk).exists())
