...
class MetricAPITests(APITestCase):
    ...
    def test_get_all_metrics(self):
        response = self.client.get(get_list_create_url())
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 4)
        returned_ids = {metric["id"] for metric in response.data["results"]}
        expected_ids = {
            self.metric.id,
    ...
    def test_get_user_metrics(self):
        response = self.client.get(get_list_create_url("user"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_admin_metrics(self):
        response = self.client.get(get_list_create_url("admin"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 2)
        returned_ids = {metric["id"] for metric in response.data["results"]}
    ...
    def test_get_owned_metric(self):
        response = self.client.get(get_details_url(self.metric.id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["id"], self.metric.id)
        self.assertEqual(response.data["name"], self.metric.name)

    def test_get_admin_metric(self):
        response = self.client.get(get_details_url(self.metric_admin.id))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["id"], self.metric_admin.id)
        self.assertEqual(response.data["name"], self.metric_admin.name)

    def test_get_other_user_metric(self):
        response = self.client.get(get_details_url(self.metric_other_user.id))
    ...
    def test_create_metric(self):
        ...
        self.assertEqual(response.data["name"], self.new_metric_data["name"])
    ...
    def test_delete_owned_metric(self):
        response = self.client.delete(get_details_url(self.metric.pk))
        self.assertEqual(response.status_code, 204)
        self.assertFalse(Metric.objects.filter(pk=self.metric.id).exists())

    def test_delete_admin_metric(self):
        response = self.client.delete(get_details_url(self.metric_admin.pk))
        self.assertEqual(response.status_code, 403)
        self.assertTrue(
